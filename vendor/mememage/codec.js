import { rsDecode, gfMul, gfPow } from "./rs.js";
import { SIG_ROWS,HEADER_BAND,HEADER_PIXELS,FOOTER_PIXELS,PIXELS_PER_BIT,PIXELS_PER_BIT_NARROW,PIXELS_PER_BIT_MAX,BAR_DELTA,LOCAL_CONTEXT_ROWS,RS_NSYM,RGB_THRESHOLD,ASYM_ENCODE,ASYM_DELTA,ASYM_FLOOR,ASYM_BOX_RADIUS,ASYM_SCALE_CAP,EVENFILL_MIN_BYTES,EVENFILL_MAX_BYTES } from "./constants.js";
// =====================================================================
...
// Mirrors bar.py's _row_color_runs / _bar_span_candidates / _scan_anywhere /
// _scan_for_bars / extract_bars. The M/Y/C↔C/Y/M bands are a distinctive
// "data begins/ends here" fiducial; these helpers find the header AND footer at
// any row and any horizontal offset, so a bar whose canvas was extended
// (margins) or that was pasted into a larger image still decodes. Magic bytes +
// CRC-16 + Reed-Solomon reject false band matches downstream.
var HSPAN_MIN_RUN=3;   // min consecutive px per band segment (JPEG erodes 8 -> ~4)

function _isM(r,g,b){return r>130&&g<120&&b>130;}
function _isY(r,g,b){return r>130&&g>130&&b<120;}
function _isC(r,g,b){return r<120&&g>130&&b>130;}

function _rowColorRuns(px,w,y){
  // Maximal M/Y/C runs on row y as [{c,start,len}]; any non-band pixel breaks
  // a run and is skipped. Mirrors bar.py:_row_color_runs.
  var runs=[],x=0;
  while(x<w){
    var i=(y*w+x)*4, r=px[i],g=px[i+1],b=px[i+2],pred=null,c=null;
    if(_isM(r,g,b)){c='M';pred=_isM;}
    else if(_isY(r,g,b)){c='Y';pred=_isY;}
    else if(_isC(r,g,b)){c='C';pred=_isC;}
    else{x++;continue;}
    var j=x+1;
    while(j<w){
      var k=(y*w+j)*4;
      if(!pred(px[k],px[k+1],px[k+2]))break;
      j++;
    }
    runs.push({c:c,start:x,len:j-x});
    x=j;
  }
  return runs;
}

function _barSpanCandidates(px,w,h,y){
  // Candidate [x0,x1] bar spans on row y from its bands, widest (most
  // band-like) first; CRC+RS pick the real bar. Mirrors bar.py:_bar_span_candidates.
  if(y<SIG_ROWS||y>=h||w<2*HSPAN_MIN_RUN*3)return [];
  var runs=_rowColorRuns(px,w,y).filter(function(r){return r.len>=HSPAN_MIN_RUN;});
  if(runs.length<6)return [];   // need header (M,Y,C) + footer (C,Y,M)
  function adjacent(i,seq){
    for(var k=0;k<3;k++)if(runs[i+k].c!==seq[k])return false;
    for(var k2=0;k2<2;k2++){
      var gap=runs[i+k2+1].start-(runs[i+k2].start+runs[i+k2].len);
      if(gap<0||gap>4)return false;
    }
    return true;
  }
  function width(i){return runs[i].len+runs[i+1].len+runs[i+2].len;}
  var headers=[],footers=[];
  for(var i=0;i<runs.length-2;i++){
    if(adjacent(i,['M','Y','C']))headers.push(i);
    if(adjacent(i,['C','Y','M']))footers.push(i);
  }
  headers.sort(function(a,b){return width(b)-width(a);});
  footers.sort(function(a,b){return width(b)-width(a);});
  var out=[],seen={};
  for(var hi2=0;hi2<headers.length;hi2++){
    var x0=runs[headers[hi2]].start;
    for(var fi=0;fi<footers.length;fi++){
      var fr=runs[footers[fi]+2];
      var x1=fr.start+fr.len-1;   // rightmost M pixel of footer
      var key=x0+','+x1;
      if(x1-x0+1>=2*HSPAN_MIN_RUN*3&&!seen[key]){seen[key]=1;out.push([x0,x1]);}
    }
  }
  return out;
}

function _cropRect(px,w,x0,y0,x1,y1){
  // Copy the inclusive rect [x0..x1]x[y0..y1] into a fresh flat RGBA array
  // (an x-crop changes the row stride, so a view can't work). Mirrors PIL crop.
...
function _scanForBars(px,w,h,firstOnly,fast){
  // Edge-anchored vertical scan: every row carrying the M/Y/C band signature
  // whose read decodes, bottom-up, de-duped. Gate on the SCALE-AWARE band
  // measurer (mirrors bar.py's _detect_bar gate), NOT the fixed-scale detectBar
  // spot check — on a resized image the bands aren't 8px wide, so fixed sample
  // points miss and the scan would never run (a 2x upscale decoded in Python
  // but returned null here). CRC+RS still guard any false-positive row.
  // Mirrors bar.py:_scan_for_bars; passing a reduced h reads a higher row pair
  // with NO pixel copy.
  var out=[],seen={};
  var b=h-1;
  while(b>=SIG_ROWS){
    if(detectBarBands(px,w,b+1)){
      var rr=_extractBarAtBottom(px,w,b+1,fast);
      if(rr){
        var key=rr.identifier+' '+rr.content_hash;
        if(!seen[key]){
          // bbox in scanned pixels: edge-anchored bars span the full width. left/right
          // let a consumer place UI on the bar's actual position (an offset/pasted bar
          // isn't at the canvas edge) rather than assuming the image edge.
          seen[key]=1;rr.bottomRow=b;rr.left=0;rr.right=w-1;out.push(rr);
          if(firstOnly)return out;
          b-=SIG_ROWS;   // skip this bar's partner (top) row
          continue;
        }
      }
    }
...
function _scanAnywhere(px,w,h,firstOnly,fast){
  // Fallback: find the bar by its band signature ANYWHERE on the canvas — any
  // row AND any horizontal offset/width — then crop to that span so the
  // bottom-path decode reads it flush (the rows above ride along, so the asym
  // reference row lands where it does at a true bottom). Heavier, so callers
  // run it only after the fast paths fail; a total-attempt cap bounds
  // pathological inputs. Mirrors bar.py:_scan_anywhere.
  var out=[],seen={},attempts=0;
  var y=h-1;
  while(y>=SIG_ROWS){
    var hit=false;
    var cands=_barSpanCandidates(px,w,h,y);
    for(var ci=0;ci<cands.length;ci++){
      attempts++;
      if(attempts>500)return out;
      var c=_cropRect(px,w,cands[ci][0],0,cands[ci][1],y);
      var res=_extractBarAtBottom(c.px,c.w,c.h,fast);
      if(res){
        hit=true;
        var key=res.identifier+' '+res.content_hash;
        if(!seen[key]){
          // bbox in scanned pixels — the span this bar was actually cropped from, so
          // an offset/pasted bar's real left/right (not the canvas edge) is known.
          seen[key]=1;res.bottomRow=y;res.left=cands[ci][0];res.right=cands[ci][1];out.push(res);
          if(firstOnly)return out;
        }
        break;
      }
...
function extractBars(px,w,h){
  // Find ALL valid bars via both scans, de-duplicated, bottom-most first —
  // edge-anchored hits first (cheaper), then the anywhere fallback adds the
  // off-position ones it didn't already surface. Mirrors bar.py:extract_bars.
  var out=[],seen={};
  var edge=_scanForBars(px,w,h,false,false);
  var any=_scanAnywhere(px,w,h,false,false);
  var all=edge.concat(any);
  for(var i=0;i<all.length;i++){
    var key=all[i].identifier+' '+all[i].content_hash;
    if(!seen[key]){seen[key]=1;out.push(all[i]);}
  }
  return out;
}
...
function extractBarScaleAware(px,w,h,scan,fast){
  ...
  // pick it up. Last resort: the full-canvas band search (a bar pasted into a
  // larger image / extended margins). scan defaults on. CRC+RS self-select per
  // candidate. Mirrors bar.py:extract_bar.
  ...
  if(r){ r.bottomRow=h-1; r.left=0; r.right=w-1; return r; }
  if(scan!==false){
    var hits=_scanForBars(px,w,h,true,fast);
    if(hits.length)return hits[0];
    var any=_scanAnywhere(px,w,h,true,fast);
...
// --- Mememage shared library: public decode API (bodies above verbatim from docs/js/codec.js) ---
export { extractBarScaleAware, extractBars, decodePayload, packPayload, embedBarPayload, extractIdentifier, normalizeIdentifier };
